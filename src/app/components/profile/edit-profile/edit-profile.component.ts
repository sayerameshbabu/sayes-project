...
export class EditProfileComponent implements OnInit {
  ...
  studentDetails: any;
  defaultImage: any = 'assets/images/user1.jpg';
  constructor(
  ...
  update() {
    let isSameFields: any = false;
    for (const key in this.editProfileForm.value) {
      if (this.studentDetails[key] !== this.editProfileForm.value[key]) {
        isSameFields = true;
        break;
      }
    }
    if (this.editProfileForm.valid && isSameFields) {
      ...
      this.toastrService.error('Please update any field to submit');
    }
  }
  onSelectFile(event) {
    if (event.target.files && event.target.files[0]) {
      ...
      if (['image/png', 'image/jpeg', 'image/gif', 'image/bmp'].includes(file.type)) {
        console.log(Math.round(file.size));
